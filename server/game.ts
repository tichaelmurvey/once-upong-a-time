...
export function launchBall(state: GameState, towardPlayer?: 1 | 2): void {
    ...
    state.ball.vx = Math.cos(angle) * BALL_STARTING_SPEED * dir;
    state.ball.vy = Math.sin(angle) * BALL_STARTING_SPEED;
...
function reflectOffPaddle(
    ...
    const speed = Math.min(currentSpeed + BALL_ACCEL, MAX_BALL_SPEED);
