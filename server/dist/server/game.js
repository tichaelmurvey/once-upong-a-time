import { FIELD_W, FIELD_H, PADDLE_SPEED, PADDLE_X_OFFSET, PADDLE_W, BALL_SIZE, BALL_SPEED, MAX_BALL_SPEED, MAX_WORD_LENGTH, } from "../shared/constants.js";
...
export function launchBall(state, towardPlayer) {
...
export function handleTypeChar(state, playerIndex, char) {
    ...
    if (!/^[a-z]$/.test(char))
        return;
    if (paddle.word.length >= MAX_WORD_LENGTH)
        return;
    paddle.word += char;
}
export function handleBackspace(state, playerIndex) {
    const paddle = state.paddles[playerIndex];
    if (state.status !== "playing")
        return;
    if (!paddle.canType)
...
export function tick(state, inputs) {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        if (inputs[i] === "up")
            state.paddles[i].y -= PADDLE_SPEED;
        if (inputs[i] === "down")
            state.paddles[i].y += PADDLE_SPEED;
    ...
    if (p1h > 0 &&
        ...
        if (state.paddles[0].word)
            state.words.push(state.paddles[0].word);
        state.paddles[1].word = "";
        state.paddles[1].canType = true;
    ...
    if (p2h > 0 &&
        ...
        if (state.paddles[1].word)
            state.words.push(state.paddles[1].word);
        state.paddles[0].word = "";
        state.paddles[0].canType = true;
...
function reflectOffPaddle(ball, paddleY, paddleH, directionX) {
